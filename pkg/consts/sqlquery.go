...
const (
	...
	SELECTPostsFlat = "SELECT p.author, p.created, p.forum, p.id, p.isEdited, p.message, p.parent, p.thread " +
		"FROM forum.post as p WHERE p.thread = $1 ORDER BY p.id LIMIT $2"
...
") " +
"select author, created, forum, id, isEdited, message, parent, thread from temp1 ORDER BY PATH LIMIT $2;"
